...
void example7()
{
    ...
    // Input file available from https://github.com/nokiatech/heif_conformance
    const char* filename = "IMG_3574.HEIC";
    if (reader->initialize(filename) == ErrorCode::OK)
    {
...
void hiddenmaster()
{
	// create reader and writer instances
	auto* reader = Reader::Create();
	auto* hidereader = Reader::Create();
	auto* writer = Writer::Create();

	// partially configure writer output
	OutputConfig writerOutputConf{};
	writerOutputConf.fileName = "auxi.heic";
	writerOutputConf.progressiveFile = true;

	ErrorCode err;
	const char* filetohidename = "summer_1440x960.heic";
	const char* filename = "autumn_1440x960.heic";

	ImageId outputPrmary;
	...
	if (reader->initialize(filename) == ErrorCode::OK)
	{
		...
		if (writer->initialize(writerOutputConf) == ErrorCode::OK)
		{
			...
			// Image Rotation property as an example of Image Properties:
			//map<PropertyId, Rotate> imageRotationProperties;  //rotation
			...
			// add hidden image
			if (hidereader->initialize(filetohidename) == ErrorCode::OK)
			{

				// read major brand of file and store it to writer config.
				FourCC hinputMajorBrand{};
				hidereader->getMajorBrand(hinputMajorBrand);

				// get information about all input file content
				FileInformation fileInfo{};
				hidereader->getFileInformation(fileInfo);

				// go through all items in input file and store master image decoder configs
				for (const auto& himage : fileInfo.rootMetaBoxInformation.itemInformations)
				{
					if (himage.features & ItemFeatureEnum::IsPrimaryImage)
					{
						cout << "is primary" << endl;
						// read image decoder config and store its id if not seen before
						DecoderConfiguration hinputdecoderConfig{};
						err = hidereader->getDecoderParameterSets(himage.itemId, hinputdecoderConfig);
						cout << "getdecodeparams " << int(err) << endl;

						if (!inputToOutputDecoderConfigIds.count(hinputdecoderConfig.decoderConfigId))
						{
							// feed new decoder config to writer and store input to output id pairing information
							DecoderConfigId houtputDecoderConfigId;
							writer->feedDecoderConfig(hinputdecoderConfig.decoderSpecificInfo, houtputDecoderConfigId);
							inputToOutputDecoderConfigIds[hinputdecoderConfig.decoderConfigId] = houtputDecoderConfigId;
						}


						// read image data
						Data himageData{};
						himageData.size = himage.size;
						himageData.data = new uint8_t[himageData.size];
						err = hidereader->getItemData(himage.itemId, himageData.data, himageData.size, false);
						cout << "getitemdata " << int(err) << endl;

						// feed image data to writer
						himageData.mediaFormat = MediaFormat::HEVC;
						himageData.decoderConfigId = inputToOutputDecoderConfigIds.at(hinputdecoderConfig.decoderConfigId);
						MediaDataId houtputMediaId;
						err = writer->feedMediaData(himageData, houtputMediaId);
						cout << "feedmediadata " << int(err) << endl;
						delete[] himageData.data;

						// create new image based on that data:
						ImageId houtputImageId;
						err = writer->addImage(houtputMediaId, houtputImageId);
						cout << "addimage " << int(err) << endl;
						//writer->setImageHidden(houtputImageId, true);
						err = writer->addAuxiliaryReference(houtputImageId, outputPrmary);
						cout << "addaux " << int(err) << endl;
						//PropertyId auxpropid;
						//Array<char> a;
						//a = makeArray<char>("d");
						//"urn:mpeg:hevc:2015:auxid:1";
						//HEIF::AuxiliaryType auxtype = { "urn:mpeg:hevc:2015:auxid:1" ,}
						//err = writer->addProperty(auxtype, auxpropid);
...
void cropped()
{
	...
	// partially configure writer output
	OutputConfig writerOutputConf{};
	writerOutputConf.fileName = "cropped.heic";
	const char* filename = "autumn_1440x960.heic";
	writerOutputConf.progressiveFile = true;
	HEIF::ErrorCode err;
		...
		if (writer->initialize(writerOutputConf) == ErrorCode::OK)
		{
			...
			for (const auto& image : fileInfo.rootMetaBoxInformation.itemInformations)
			{
				if (image.features & ItemFeatureEnum::IsPrimaryImage)
				{
					...
					// rotate code - works
					const HEIF::Rotate rot{ 90 };
					PropertyId rotid;
					//err = writer->addProperty(rot, rotid);
					//cout << "add property " << int(err) << endl;
					//err = writer->associateProperty(outputImageId, rotid, isessential);
					//cout << "associate proeprty " << int(err) << " - is essential?  " <<std::boolalpha << isessential << endl;


					// aperture code
					//const CleanAperture cropparams{ 400, 1, 100, 1, 0, 1, 0, 1 };
					//PropertyId cropid;
					//err = writer->addProperty(cropparams, cropid);
					//cout << "add property " << int(err) << endl;
 					//err = writer->associateProperty(outputImageId, cropid, isessential);
					//cout << "associate proeprty " << int(err) << " - is essential?  " <<std::boolalpha << isessential << endl;


					// aperture code
					const HEIF::Mirror mir{ false };
					PropertyId mirrorid;
					err = writer->addProperty(mir,mirrorid);
					cout << "add property " << int(err) << endl;
					err = writer->associateProperty(outputImageId, mirrorid, isessential);
					cout << "associate proeprty - mirror " << int(err) << " - is essential?  " << std::boolalpha << isessential << endl;
...
void thumb()
{
	...
	// partially configure writer output
	OutputConfig writerOutputConf{};
	writerOutputConf.fileName = "thumb.heic";
	writerOutputConf.progressiveFile = true;
					...
					if (hidereader->initialize(filetohidename) == ErrorCode::OK)
					{
						...
						for (const auto& himage : fileInfo.rootMetaBoxInformation.itemInformations)
						{
							if (himage.features & ItemFeatureEnum::IsPrimaryImage)
							{
								...
								err = writer->addThumbnail(houtputImageId, outputImageId);
								...
								if (hidereader->initialize(filetohidename) == ErrorCode::OK)
								{
									...
									for (const auto& himage : fileInfo.rootMetaBoxInformation.itemInformations)
									{
										if (himage.features & ItemFeatureEnum::IsPrimaryImage)
										{
											...
											err = writer->addThumbnail(houtputImageId, outputImageId);
											cout << "addthumb " << int(err) << endl;


											writer->setImageHidden(houtputImageId, true);
											cout << "hdiethumb " << int(err) << endl;
		...
		if (writer->initialize(writerOutputConf) == ErrorCode::OK)
		{
			...
			for (const auto& image : fileInfo.rootMetaBoxInformation.itemInformations)
			{
				if (image.features & ItemFeatureEnum::IsMasterImage)
				{
					...
					if (hidereader->initialize("winter_1440x960.heic") == ErrorCode::OK)
					{
						...
						for (const auto& himage : fileInfo.rootMetaBoxInformation.itemInformations)
						{
							if (himage.features & ItemFeatureEnum::IsPrimaryImage)
							{
								...
								himageData.mediaFormat = MediaFormat::HEVC;
								himageData.decoderConfigId = inputToOutputDecoderConfigIds.at(hinputdecoderConfig.decoderConfigId);
								MediaDataId thoutputMediaId;
								err = writer->feedMediaData(himageData, thoutputMediaId);
								cout << "feedmediadata " << int(err) << endl;
								delete[] himageData.data;

								// create new image based on that data:
								ImageId thoutputImageId;
								err = writer->addImage(thoutputMediaId, thoutputImageId);

								err = writer->addThumbnail(thoutputImageId, outputImageId);

								cout << "addthumb " << int(err) << endl;
								//writer->setImageHidden(thoutputImageId, true);
								//err = writer->addAuxiliaryReference(houtputImageId, outputPrmary);
								//cout << "addaux " << int(err) << endl;
								//PropertyId auxprop;
			...
			writer->finalize();
			cout << "written to " << writerOutputConf.fileName << endl;
		}
	}
	else
	{
...
int main()
{
	...
	hiddenmaster();
	//cout << "Run cropped:" << endl;
	//cropped();
	//cout << "Aux" << endl;
	//aux();
	//cout << "thumb" << endl;
	//thumb();
	//cout << "multithumb" << endl;
	//multithumb();
}
